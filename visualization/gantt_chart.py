...
def plot(JobShop):
    ...
    for machine in JobShop.machines:
        ...
        for operation in machine_operations:
            ...
            if(operation.is_broken == True): 
                is_broken_info = operation.is_broken_info
                operation_start = is_broken_info['old_start']
                operation_end = is_broken_info['old_end']
                operation_duration = is_broken_info['percentage_done']*operation.processing_times[is_broken_info['broken_machine']]
                operation_label = f"{operation.operation_id}"
                operation_machine = is_broken_info['broken_machine']
                # Set color based on job ID
                color_index = operation.job_id % len(JobShop.jobs)
                if color_index >= colormap.N:
                    color_index = color_index % colormap.N
                ...
                ax.broken_barh(
                    [(operation_start, operation_duration)],
                    (operation_machine - 0.4, 0.8),
                    facecolors=color,
                    edgecolor='black'
                )
                ax.text(
                    operation_start + operation_duration / 2,
                    operation_machine,
                    operation_label,
                    ha="center",
                    va="center",
