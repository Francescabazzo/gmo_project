...
class Operation:
    ...
    @property
    def is_broken_info(self): 
        return self._isbroken_info 

    def set_broken(self, time_broken):
        self._isbroken = True 
        if self.scheduled_start_time < time_broken: 
            processed_time_done = time_broken - self.scheduled_start_time
            processing_to_do = self._processing_times[self.scheduled_machine] - processed_time_done
            self._isbroken_info['processed_time_done'] = processed_time_done
            self._isbroken_info['processing_to_do'] = processing_to_do
            self._isbroken_info['broken_machine'] = self.scheduled_machine
            self._isbroken_info['percentage_done'] = processed_time_done/self.processing_times[self.scheduled_machine]
            self._isbroken_info['percentage_to_do'] = processed_time_done/self.processing_times[self.scheduled_machine]
            self._isbroken_info['old_start'] = self.scheduled_start_time
            self._isbroken_info['old_end'] = time_broken 
    ...
    #   Property added
    def update_processing_time(self, machine_id, duration) -> None:
