...
def run_GA(jobShopEnv, population, toolbox, stats, hof, **kwargs):
    ...
    # Fitness progress to save the data and then plot 
    fitness_progress = [] 
    ...
    fitness_progress.append((current_time, best_fitness, gen))
    logging.info(f"Generation {gen}: Time {current_time:.2f}s - Best fitness = {best_fitness}")
    ...
    for gen in range(1, kwargs['algorithm']['ngen'] + 1):
        ...
        current_time = time.time() - start_time 
        current_best_fitness = hof[0].fitness.values[0] if hof else float('inf')
        fitness_progress.append((current_time, current_best_fitness, gen))

    total_time = time.time()-start_time 
    makespan, jobShopEnv = evaluate_individual(hof[0], jobShopEnv, reset=False)
    logging.info(f"Makespan: {makespan}")

    return makespan, jobShopEnv, fitness_progress 
