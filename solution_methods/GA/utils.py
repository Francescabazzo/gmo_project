...
def plot_fitness_progress_time(fitness_progress, save_path=None, show_plot=True):
    ...
    """
    times = [item[0] for item in fitness_progress]
    fitness_values = [item[1] for item in fitness_progress]
    plt.figure()
    plt.plot(times, fitness_values)
    plt.title('Evolution of the makespan during optimization (GA)', fontsize=12)
    plt.xlabel("Computation Time (seconds)", fontsize=11)
    plt.ylabel("Makespan", fontsize=11)
...
def plot_fitness_progress_gen(fitness_progress, save_path=None, show_plot=True):
    ...
    """
    generations = [item[2] for item in fitness_progress]
    fitness_values = [item[1] for item in fitness_progress]
    plt.figure()
