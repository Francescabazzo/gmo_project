...
class FJS_reschedule(JobShop):
    ...
    def create_model(self):
        ...
        for i in self.operations_toprocess:
            ...
            model.addConstr(
                C[i] == S[i] + gb.quicksum(
                    X[i, k] * operation.processing_times[k]
            ...
            if idx > 0:  # the operation has a predecessor
                ...
                if pred.operation_id in self.operations_completed + self.operations_ongoing:
                    ...
                    model.addConstr(
                        ...
                        gb.quicksum(
                            pred.processing_times[k] * X[pred.operation_id, k] for k in pred.optional_machines_id),
    ...
    def run_model(self, time_limit=None, X_start=None, S_start=None, C_start=None, Y_start=None, E_start=None,
                  Z_start=None, Z_aux_start=None):
        """
        ...
        if time_limit is not None:
            time_phase_1 = time_limit * 0.6
            time_phase_2 = time_limit * 0.3
            time_phase_3 = time_limit * 0.1
        ...
        # Phase 1: optimization of the first objective
        print("Phase 1: Optimization of the first objective.")

        self.incumbent.start_phase(1)

        # The first objective is to minimize the total delay after the rescheduling.
        self.model.setObjective(self.obj_quadr, gb.GRB.MINIMIZE)

        if time_phase_1 is not None:
            self.model.Params.TimeLimit = time_phase_1
        ...
        if self.model.Status == gb.GRB.OPTIMAL or self.model.Status == gb.GRB.TIME_LIMIT:
            obj1_value = self.model.objVal

            # Phase 2: Optimization machine assignments
            # The second objective is set as the objective of the model, and the value of the first objective is set as a constraint.

            print("Phase 2: Optimization cmax")

            self.incumbent.start_phase(2)

            tolerance = 0.10  # Tolerance on how "bad" the first objective can become.

            # Additional constraint: value of the first objective
            self.model.addConstr(
                self.obj_quadr <= obj1_value * (1 + tolerance),
                name="fix_quadratic_obj"
            )

            # Objective: minimization of cmax
            self.model.setObjective(
                self.obj_mach_assignments, gb.GRB.MINIMIZE
            )

            if time_phase_2 is not None:
                self.model.Params.TimeLimit = time_phase_2
            ...
            if self.model.Status == gb.GRB.OPTIMAL or self.model.Status == gb.GRB.TIME_LIMIT:
                obj2_value = self.model.objVal

                # Phase 3: Optimization cmax
                # The third objective is set as the objective of the model, and the value of the first and second
                # objective are set as constraints.

                self.incumbent.start_phase(3)
                ...
                self.model.addConstr(
                    self.obj_mach_assignments <= obj2_value * (1 + tolerance_2)
                )
                ...
                if time_phase_3 is not None:
                    self.model.Params.TimeLimit = time_phase_3
