...
def run_nsga2(exp_num: int, n_pop: int, n_gen: int, file_path):
    """
    Simulation of NSGA-II with the specified parameters.

    Args:
        - exp_num: number of experiments
        - n_pop: number of individuals in a population
        - n_gen: number of generations
        - file_path: path of the environment with the plan to reschedule
    """
    ...
    actual_log_dir = SAVE_LOGS / f"EX{exp_num}" / f"Pop_{n_pop}" / f"test{n_gen}"
    os.makedirs(actual_log_dir, exist_ok=True)

    logger = setup_experiment_logging(
        log_dir=SAVE_LOGS,  # Base Directory
        exp_num=exp_num,
        log_filename=str(actual_log_dir / f"experiment_nsga_ex{exp_num}.txt")
    )

    terminal_output_file = actual_log_dir / f"terminal_output_nsga_ex{exp_num}.txt"
    ...
    try:
        ...
        results_dir = os.path.join(SAVE_DIR, "NSGA2", f"EX{exp_num}", f"Pop_{n_pop}", f"test{n_gen}")
        ...
        with open(terminal_output_file, 'w', encoding='utf-8') as term_file:
            ...
            with redirect_stdout(tee_out), redirect_stderr(tee_err):
                ...
                term_file.write(f"Generations: {n_gen}\n")
                ...
                try:
                    ...
                    initializer = RandomInitializer(jobShopEnv, disruption_time, broken_machine_id)
...
if __name__ == "__main__":
    for f in range(1, 26):
        ...
        for pop in [20, 50, 100, 200]:
            for n_gen in [500, 1000, 2000, 5000]:
                run_nsga2(exp_num, pop, n_gen, os.path.join(RESULTS_SCH_DIR, f"schedule_simulation_{exp_num}.pkl"))